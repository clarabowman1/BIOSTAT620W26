...
state_pops <- state_pops %>%
  rename(abb = state, state=state_name)
### FINISH THE CODE HERE
...
There are 58094 observations and no null values. State, date, state and state abbreviation are strings. All other variables are numeric
...
Data looks like it is correctly formatted and ranges seem appropriate. Maximum cases and deaths seems high. Data ranges from January 1, 2020 to March 3, 2023.
...
### COMPLETE CODE HERE
p1 <- ggplot(cv_states, aes(y=new_cases)) +
  geom_boxplot() +
  labs(title = "Distribution of New Cases")
ggplotly(p1)

p2 <- ggplot(cv_states, aes(y=new_deaths)) +
  geom_boxplot() +
  labs(title = "Distribution of New Deaths")
ggplotly(p2)
...
There are far fewer deaths than cases, but they follow a similar trend. Deaths usually reach peaks about one month after cases reach a peak.
...
# Create hover text to include state name, new cases per 100k, cases, and deaths
cv_per100$hover <- with(cv_per100, paste(state.name, '<br>', 
